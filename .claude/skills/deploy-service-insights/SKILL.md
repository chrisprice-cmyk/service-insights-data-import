---
name: deploy-service-insights
description: Run the Service Insights Data Import tool against a Salesforce org — generate demo Case data and confirm it renders in CRM Analytics "Service Analytics" and/or Tableau Next "Service Insights". Use when a user asks to deploy, run, or set up this data generator against an org.
---

# Deploy Service Insights demo data

This skill walks an SE through running `service_insights_data_import`
...
the kind of action the workspace's risk policy requires pausing for, even
though the tool's own `BatchGuard` makes it safely undoable.
